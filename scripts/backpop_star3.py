...
def evolv2(params_in, stage, params_out):
    ...
    if stage == 'BNS_merger':
        ...
        out = bpp.loc[((bpp.kstar_1 == 14) & (bpp.kstar_2.isin([7,8,9])) & (bpp.sep > 0)) |
                      ((bpp.kstar_1.isin([7,8,9])) & (bpp.kstar_2 == 14) & (bpp.sep > 0))].groupby('bin_num', as_index=False).first()
    elif stage == "NS_He":
        out = bpp.loc[((bpp.kstar_1 == 13) & (bpp.kstar_2.isin([7,8,9])) & (bpp.sep > 0)) |
                      ((bpp.kstar_1.isin([7,8,9])) & (bpp.kstar_2 == 13) & (bpp.sep > 0))].groupby('bin_num', as_index=False).first()
    elif stage == "WD_He":
        out = bpp.loc[((bpp.kstar_1.isin([10,11,12])) & (bpp.kstar_2.isin([7,8,9])) & (bpp.sep > 0)) |
                      ((bpp.kstar_1.isin([7,8,9])) & (bpp.kstar_2.isin([10,11,12])) & (bpp.sep > 0))].groupby('bin_num', as_index=False).first()
...
#Porb: 4.17 hr +/- 0.05 (0.17375 days)
#Eccentricity: 0.0  +/- 0.01 
#MBH: min is 1.18 
#Mstar: spectroscopic: 5.32 + 15.75 - 3.83, evolutionary: 3.38 + 2.86 - 0.42
mean = np.array([3.4, 0.17375, 0.0])
cov = np.array([[2.0, 0, 0], [0, 0.005**2, 0], [0, 0, 0.001]])
...
param_names = ["m1", "m2", "tb", "e", "alpha1_2", "vk1", "theta1", "phi1"]
lower_bound = np.array([m1lo, m2lo, tblo, elo, alphalo_2, vklo, thetalo, philo])
upper_bound = np.array([m1hi, m2hi, tbhi, ehi, alphahi_2, vkhi, thetahi, phihi])
...
np.savez('../data/stripped/fixed_facc_BH_He',nwalkers=n_walkers,n_steps=n_steps,chain=sampler.chain)
